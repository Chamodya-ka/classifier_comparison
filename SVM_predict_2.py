...
svm_dir = "./models/SVM/SVM_II.sav"
svm = pickle.load(open(svm_dir, 'rb'))
...
with mp_pose.Pose(
    ...
    min_tracking_confidence=0.4) as hands:
      ...
      while cap.isOpened():
        ...
        kp_scaled = scale.fit_transform(kp_lst)
        print(kp_scaled.shape)
        
        #print(kp_scaled.shape)
        #prediction = model.predict(kp_lst)
        result = svm.predict(kp_lst)
        print(result , result.shape)
        #guess = np.argmax(prediction)
        
        cv2.putText(annotated_image,str(result) + ", confidence: NaN"  , (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0),2)
